...
- [ ] RQ-OPS-014: 無料運用で背景Push通知を定期実行できる
  ...
  - 対応タスク: BL-038
  - 証跡: 未記入
- [ ] RQ-OPS-015: 背景Push dispatch の定期実行を無料枠環境でも安定稼働させる
...
- [ ] RQ-POM-007: iOS PWA で背景中のセッション通知を受け取れる
  ...
  - 対応タスク: BL-038
  - 証跡: 未記入
- [ ] RQ-POM-008: iOS PWA 復帰時にサーバー時刻基準で表示タイマーを再同期できる
  - 受け入れ条件: iOS で別アプリから復帰した直後に `/api/v1/pomodoro/current` を再取得し、表示タイマーを再同期できる
  - 受け入れ条件: ローカル秒減算の停止/遅延が発生しても、復帰後に実時間との差分が残らない
  - 受け入れ条件: 復帰イベント（`visibilitychange` / `focus` / `pageshow`）で再同期を実行しても過剰リクエストを抑制できる
  - 対応タスク: BL-039
  - 証跡: 未記入
